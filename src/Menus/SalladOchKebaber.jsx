...
export default function SalladOchKebaber({ active, setActiveCategory }) {
    const category = "salladOchKebaber"
    const salladOchKebaber = data.filter((item) => item.kategori === category)
        ...
        >
            <div>
                ...
                <h2 className="menu-info">
                    Husets lyxiga sallader med
                    <br />
                    nybakat bröd och valfri sås
                </h2>
            </div>
            {salladOchKebaber.map((item, index) => (
                <li
                    key={index}
                    className="menu-content"
                >
                    <h3 className="number">{index + 23}.</h3>
